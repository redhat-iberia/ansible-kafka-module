...
failed:
  ...
  sample: false
changed:
  description: Whether the module caused a change (a message was produced).
  type: bool
  returned: always
  sample: true
topic:
  description: Topic where the message was produced.
  type: str
  returned: success
  sample: my_topic
partition:
  ...
  sample: 0
offset:
  description: Offset of the produced message in the partition.
  type: int
  returned: success
  sample: 42
timestamp:
  description: Timestamp assigned to the produced message (milliseconds since epoch).
  type: int
  returned: success
  sample: 1609459200000
message_details:
  description: Metadata about the sent message.
  type: dict
  returned: success
  contains:
    has_key:
      description: Whether a message key was provided.
      type: bool
      sample: true
    has_headers:
      description: Whether headers were provided with the message.
      type: bool
      sample: false
    message_size:
      ...
      sample: 128
error_type:
  description: Machine-readable error type when the module fails.
  type: str
  returned: failure
  sample: broker_unavailable
message:
  description: Human-readable error message when the module fails.
  type: str
  returned: failure
  sample: "No brokers available"
"""
...
def main():
    ...
    if security_protocol in ["SASL_PLAINTEXT", "SASL_SSL"]:
        ...
        if any(sasl_params) and not all(sasl_params):
            module.fail_json(
                msg="When configuring SASL, all of sasl_mechanism, sasl_username, and sasl_password must be provided"
            )
        if all(sasl_params):
